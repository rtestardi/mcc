...
// *****************************************************************************
/* MISRA C-2012 Rule 10.1 deviate: , and 10.4 deviate:2. Deviation record ID -  
...
)
{
    ...
    if((handle != DRV_HANDLE_INVALID) && (handle != (DRV_HANDLE)(NULL)) && (pUSBDrvObj->isOpened == true))
    {
        ...
        if(pUSBDrvObj->operationMode == USB_OPMODE_DEVICE)
        {
            ...
            pUSBDrvObj->vbusIsValid = true;  // add  // force session valid
            pUSBDrvObj->pEventCallBack(pUSBDrvObj->hClientArg, DRV_USBFS_EVENT_DEVICE_SESSION_VALID,  (void *)NULL);  // add
